...
SELECT * FROM tbMensagens AS M
 ...
 WHERE (`S`.codCliente = '".$from_user_id."' 
 AND `S`.codPrestador = '".$to_user_id."') 
...
{
 $query = "SELECT nomePrestador FROM tbprestador WHERE codPrestador = '$user_id'";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 foreach($result as $row)
 {
  return $row['nomePrestador'];
 }
}
